...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./system/premium.json", JSON.stringify(_dir));
};
 ...
 * @param {Object} _dir
 */
const expiredCheck = (IconicTechInc, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./system/premium.json", JSON.stringify(_dir));
			idny ? IconicTechInc.sendMessage(idny, { text: "Your premium has run out, please buy again." }) : "";
			idny = false;
		}
	}, 1000);
};
